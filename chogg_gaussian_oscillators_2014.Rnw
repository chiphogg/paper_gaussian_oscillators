...
\begin{abstract}
  We investigate using animations to visualize uncertainty in curves and surfaces.
...
interpolation.caption <- sprintf(
    paste("Interpolation illustrated for a standard 1-D Gaussian.",
          ...
          "(b) Pointwise quantiles for interpolating %s sets of random draws.",
...
Therefore, we assume that our results for $\text{DO}_2$ animations are a useful heuristic for the quality of the great circle animations, unless stated otherwise.\footnote{
...
Specifically, they have constructed techniques such that the distribution of all animations is marginally Gaussian at each individual time.
...
However, the properties of \textit{individual} animations are even more important; after all, visualizations contain only a single animation.
We can take the frames of an animation to define a probability distribution, which we can also compare to the Gaussian.

To see this distinction starkly, consider the simplest Gaussian oscillator: a single Gaussian draw, constant for all time.
...
This paper is written using the \texttt{knitr} package\cite{knitr}, which enabled me to include the source code for my figures directly in the text.
All source is available on github: \url{https://github.com/chiphogg/paper_gaussian_oscillators}.
