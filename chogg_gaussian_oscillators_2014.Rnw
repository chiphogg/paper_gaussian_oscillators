...
# Create the desired number of copies of each oscillator.  First, do
# independent Gaussian draws as a reference point.
independent <- lapply(equivalent_draws,
                      function(n.draws) {
                        KsOnColumns(matrix(rnorm(n=n.draws * num_oscillators),
                                           ncol=num_oscillators))
                      })
