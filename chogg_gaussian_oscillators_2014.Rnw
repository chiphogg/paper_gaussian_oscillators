...
\begin{abstract}
  ...
  First, we define the \textit{Gaussian oscillator}, a unifying concept which encompasses existing techniques (and helps develop new ones).
  We propose criteria to judge the quality of different animation techniques.
  These include the Kolmogorov-Smirnov \textit{distribution}: the first quantitative metric to measure how faithfully an animation represents its underlying distribution.
  This paper also introduces a new animation technique, the \textit{delocalized oscillator}, which combines the best traits of existing techniques: these animations feature fluid, natural motion, and also represent the underlying distribution more faithfully and fully.
  Finally, we provide a roadmap to develop even better animations in the future, by leveraging a key insight: that Gaussian animations are nothing more than a special type of Gaussian processes.
\end{abstract}
...
They also represent the normal distribution more faithfully than interpolated animations for the same number of samples, as Section \ref{sub:statistical_properties} elaborates.
Their shortcoming is that they cannot incorporate more than two samples, and therefore cannot further improve their fidelity.
...
Readers familiar with the concept may have noticed that Gaussian oscillators are also Gaussian \textit{processes}\cite{3569}, with a few constraints.
...
For these reasons, online animations are better when available.
...
\right].
\end{equation}
$\xi(t)$ is an offline, periodic technique, with period $N$.
...
This analysis assumes that the animation is composed of independent Gaussian oscillators, which is usually the case.
...
In fact, great circle animations are a subset of measure zero of the $\text{DO}_2$ family: the former always produces circles, but the latter can also produce ellipses.
Therefore, we assume that our results for $\text{DO}_2$ animations are a useful heuristic for the quality of the great circle animations, unless stated otherwise.
...
This motivates us to evaluate each family of Gaussian oscillators according to how faithfully its individual members represent the standard Gaussian distribution.
...
Having proposed criteria to judge Gaussian oscillators, we now assess their relative importance.
...
If we perform an Anscombe transform\cite{anscombe1948}, the transformed data follows the Gaussian distribution to an excellent approximation.
...
Curves sampled from this distribution are twice differentiable everywhere\cite{3569}; hence, their motion will be smooth and natural.
...
\texttt{R} packages which output interactive web apps, such as \texttt{shiny}\cite{shiny}, can be augmented to support animated output.
...
I am also thankful to Melissa DeLucchi, who provided valuable feedback on a draft of this paper.
