...
If $\xi = \left( \xi_1, \cdots, \xi_m \right)^\intercal$ represents $m$ \iid~draws from the \textit{standard} normal $N(0, 1)$, then $\alpha \equiv \mu + L\xi$ is a draw from $N(\mu, K)$.
...
Both techniques involve generating some number $N$ of \iid~standard normal variates, then using them deterministically to produce a continuous timetrace.
...
The uncertainty can be analyzed in \texttt{R}, as usual --- but rather than a static image or animated \texttt{.gif}, the output is an interactive web app.
