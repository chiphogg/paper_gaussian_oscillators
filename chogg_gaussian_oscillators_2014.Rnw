...
An important advance was made by Hennig (cite), who generated circular paths with marginal normal distributions.
...
The next three subfigures compare two types of Gaussian Oscillator\footnote{
  We cannot include the Great Circles approach of (cite Hennig), since technically they are not Gaussian Oscillators.
...
Consider a Poisson distribution (representing, e.g., neutron counts in a scattering curve\footnote{cite my paper?}).
The Anscombe transform\footnote{citey cite} takes Poisson-distributed data into Gaussian-distributed to an excellent approximation.
